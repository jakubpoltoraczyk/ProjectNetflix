...
def display_every_actor_dataset(movie):
    for index in range(movie.get_number_of_actors()):
        ...
        print(
            ...
            actor.get_surname(),
            actor.get_age(),
            actor.get_nationality(),
            actor.get_gender(),
            actor.get_photo_link(),
            actor.get_rating(),
        )
...
def main():
    ...
    director = Director(
        "Mark", "Jones", "USA", Date(11, 5, 1965), "male", "some-link.com", 8.2, 10
    )

    description = MovieDescription(
        "Harry Potter",
        "A movie about a boy who lived",
        "A movie about a boy who lived and now is studying magic in Hogward school with his friends Ron and Hermione",
    )
